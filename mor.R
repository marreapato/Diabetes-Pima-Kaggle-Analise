...
library(readODS)#for linux
setwd("~/Documents")

novmort <- read.ods(file = "//home//lucas//Documents//novmort.ods")#reading ods archive

library(fmsb)#Mortality odds ratio package
...
#n of cancer cases per age
canceridademen <- canceridademen[-1,]#invalid first row

canceridademen <- as.data.frame(canceridademen)

canceridademen <- spread(canceridademen,key = Var2,value = Freq)#spreading data

mencancernames <- canceridademen[,1]#vector of cancer names

canceridademen <- canceridademen[,-1]

canceridademen#mirrowed with cacrmen names vector
...
names(listacancermen)=mencancernames#assigning names
...
names(listaMORMEN)=mencancernames#assigning names
